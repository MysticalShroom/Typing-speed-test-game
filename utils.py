...
def calculate_accuracy(target_len: int, errors: int) -> float:
    ...
    if target_len <= 0:
        return 0.0  # Avoid division by zero, accuracy is meaningless
    # Ensure errors don't exceed target length for calculation
    actual_errors = min(errors, target_len)
    correct_chars = target_len - actual_errors
    accuracy = (correct_chars / target_len) * 100
    return round(accuracy, 1)  # Round to one decimal place
